...
def read_files_from_csv(csv_file):
    ...
    with open(csv_file, 'r') as f:
        ...
        for row in reader:
            ...
            MI=caculated_MI(msa_file1,msa_file2,ifc_seq_file)
...
hete_MI=read_files_from_csv("/home/zjlab/Sample_dir/bacteria_heteroPDBs_50sample.csv")
homo_MI=read_files_from_csv("/home/zjlab/Sample_dir/bacteria_homoPDBs_50sample.csv")


# 将 heterodimer MI 值写入文件
with open("hete_MI.txt", "w") as f:
    for mi_list in hete_MI:
        for mi in mi_list:
            f.write(str(mi) + " ")
        f.write("\n")
        
# 将 homo MI 值写入文件
with open("homo_MI.txt", "w") as f:
    for mi_list in homo_MI:
        for mi in mi_list:
            f.write(str(mi) + " ")
        f.write("\n")