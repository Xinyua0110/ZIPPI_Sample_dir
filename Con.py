...
hete_Con=read_files_from_csv("/home/zjlab/Sample_dir/bacteria_heteroPDBs_50sample.csv")
homo_Con=read_files_from_csv("/home/zjlab/Sample_dir/bacteria_homoPDBs_50sample.csv")


# 将 heterodimer Con 值写入文件
with open("hete_Con.txt", "w") as f:
    for mi_list in hete_Con:
        for mi in mi_list:
            f.write(str(mi) + " ")
        f.write("\n")
        
# 将 homo Con 值写入文件
with open("homo_Con.txt", "w") as f:
    for mi_list in homo_Con:
        for mi in mi_list:
            f.write(str(mi) + " ")
        f.write("\n")