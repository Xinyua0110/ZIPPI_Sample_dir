...
hete_DI=read_files_from_csv("/home/zjlab/Sample_dir/bacteria_heteroPDBs_50sample.csv")
homo_DI=read_files_from_csv("/home/zjlab/Sample_dir/bacteria_homoPDBs_50sample.csv")




# 将 heterodimer DI 值写入文件
with open("hete_DI.txt", "w") as f:
    for mi_list in hete_DI:
        for mi in mi_list:
            f.write(str(mi) + " ")
        f.write("\n")
        
# 将 homo DI 值写入文件
with open("homo_DI.txt", "w") as f:
    for mi_list in homo_DI:
        for mi in mi_list:
            f.write(str(mi) + " ")
        f.write("\n")